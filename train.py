...
parser.add_argument("--conf_thres", type=float, default=0.78, help="object confidence threshold")
...
for epoch in range(opt.epochs):
    ...
    with torch.no_grad():
       ...
       for batch_i, (_, imgs, targets) in enumerate(val_dataloader):
        ...
        v_recall.append(model.losses["recall"])
        v_prec.append(model.losses["precision"])
    ...
    ap = compute_ap(v_recall, v_prec)

    str = "validation [Epoch %d/%d] [recall: %.5f, precision: %.5f, ap: %.5f]\n" % (
                epoch+1,
                opt.epochs,
                val_recall,
                val_precision,
