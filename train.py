...
for epoch in range(opt.epochs):
    for batch_i, (_, imgs, targets) in enumerate(dataloader):
        ...
        str = "Training [Epoch %d/%d, Batch %d/%d] [Losses: x %f, y %f, w %f, h %f, conf %f, cls %f, total %f, recall: %.5f, precision: %.5f]" % (
                epoch,
                opt.epochs,
                batch_i,
                len(dataloader),
    ...
    for batch_i, (_, imgs, targets) in enumerate(val_dataloader):
        ...
        str = "[validation Epoch %d/%d, Batch %d/%d] [Losses: x %f, y %f, w %f, h %f, conf %f, cls %f, total %f, recall: %.5f, precision: %.5f]" % (
                epoch,
                opt.epochs,
                batch_i,
                len(dataloader),
